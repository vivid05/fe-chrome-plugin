window.onload = function () {
  ...
  const getComponentFormData = () => {
    ...
    if (!dialogWrapper || dialogWrapper.style.display === 'none') {
      document.querySelector('#app').__vue__.$message({ type: 'warning', message: '请先打开了编辑表单' })
      return null
    }
    try {
      const vm = document.querySelector('#app').__vue__
      const childElementCount = vm.$children[0].$children[0].$children[1].$children[1].$children[0].$el.childElementCount
      const cmp = childElementCount > 1 ? vm.$children[0].$children[0].$children[1].$children[1].$children[0] : vm.$children[0].$children[0].$children[1].$children[1].$children[0].$children[0]
      ...
      if (cmp._setupProxy) {
        ...
        formData = deepClone(cmp._setupProxy.formData) || deepClone(cmp.$children[childrenLength - 1]?._setupProxy?.form) || deepClone(cmp.$children[childrenLength - 1].form)
      } else {
        formData = deepClone(cmp.formData) || deepClone(cmp.$children[childrenLength - 1].form)
      }
      return formData
    } catch (error) {
      document.querySelector('#app').__vue__.$message({ type: 'warning', message: '获取表单数据失败' })
      console.warn(error)
      return null
    }
  }
  ...
  pasteBtn.onclick = () => {
    const dialogWrapper = document.querySelector('.el-dialog__wrapper')
    if (!dialogWrapper || dialogWrapper.style.display === 'none') {
      document.querySelector('#app').__vue__.$message({ type: 'warning', message: '请先打开新增表单' })
      return
    }
        ...
        if (currentInstance._setupProxy.formData) {
          ...
          if (currentInstance.$children[childrenLength - 1]._setupProxy) {
            for (let key in currentInstance.$children[childrenLength - 1]._setupProxy.form) {
              currentInstance.$children[childrenLength - 1]._setupProxy.form[key] = globalFormData[key]
            }
          } else {
  ...
  window.copyAndPastebtnWrapper.style.display = 'block'
