! function (t) {
  ...
  70: function (t, e, n) {
    ...
    function createHoverBox(x, y, imageUrl) {
      const hoverBox = createPopupBox(x, y)
      hoverBox.textContent = imageUrl;
      document.body.appendChild(hoverBox);
      // Bind the hoverBox to the body element to handle mouseout event properly
      document.body.addEventListener('mouseout', function (event) {
        if (!hoverBox.contains(event.relatedTarget)) {
          hoverBox.remove();
        }
      });
    ...
    document.addEventListener('mouseup', (event) => {
      if (event.target.id !== 'popup-box') {
        ...
        if (isValidDate(selectedText)) {
          ...
          showTimestampHoverBox(event.clientX, event.clientY, timestamp)
        } else {
          hideTimestampHoverBox()
        }
      }
